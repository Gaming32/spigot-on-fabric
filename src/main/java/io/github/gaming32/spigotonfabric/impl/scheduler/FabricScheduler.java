...
public class FabricScheduler implements BukkitScheduler {
     ...
     */
    private final AtomicReference<FabricTask> tail = new AtomicReference<FabricTask>(head);
    /**
     * Main thread logic only
     */
    private final PriorityQueue<FabricTask> pending = new PriorityQueue<FabricTask>(10,
            new Comparator<FabricTask>() {
                @Override
                public int compare(final FabricTask o1, final FabricTask o2) {
     ...
     * Main thread logic only
     */
    private final List<FabricTask> temp = new ArrayList<FabricTask>();
    /**
     * These are tasks that are currently active. It's provided for 'viewing' the current state.
     */
    private final ConcurrentHashMap<Integer, FabricTask> runners = new ConcurrentHashMap<Integer, FabricTask>();
    /**
    ...
    public <T> Future<T> callSyncMethod(final Plugin plugin, final Callable<T> task) {
        ...
        final FabricFuture<T> future = new FabricFuture<T>(task, plugin, nextId());
        handle(future, 0L);
        return future;
    }
    ...
    public List<BukkitTask> getPendingTasks() {
        final ArrayList<FabricTask> truePending = new ArrayList<FabricTask>();
        for (FabricTask task = head.getNext(); task != null; task = task.getNext()) {
            if (task.getTaskId() != -1) {
                // -1 is special code
    ...
    private void parsePending() {
        ...
        // We split this because of the way things are ordered for all of the async calls in FabricScheduler
