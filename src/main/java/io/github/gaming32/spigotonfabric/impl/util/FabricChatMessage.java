...
public final class FabricChatMessage {

    private static final Map<ChatHexColor, EnumChatFormat> COLOR_TO_LEGACY_FORMAT = ChatHexColor.LEGACY_FORMAT_TO_COLOR
        .entrySet()
        .stream()
    ...
    public static ChatColor getColor(EnumChatFormat format) {
        return ChatColor.getByChar(format.getChar());
    }
    ...
    public static String fromStringToJSON(String message, boolean keepNewlines) {
        IChatBaseComponent component = FabricChatMessage.fromString(message, keepNewlines)[0];
        return FabricChatMessage.toJSON(component);
    }

    public static String fromStringOrNullToJSON(String message) {
        IChatBaseComponent component = FabricChatMessage.fromStringOrNull(message);
        return FabricChatMessage.toJSONOrNull(component);
    }

    public static String fromJSONComponent(String jsonMessage) {
        IChatBaseComponent component = FabricChatMessage.fromJSONOrNull(jsonMessage);
        return FabricChatMessage.fromComponent(component);
    }
    ...
    public static String fromComponent(IChatBaseComponent component) {
        ...
        for (IChatBaseComponent c : SpigotOnFabric.iterateComponent(component)) {
            ChatModifier modi = c.getStyle();
            ChatHexColor color = modi.getColor();
            if (c.getContents() != LiteralContents.EMPTY || color != null) {
                if (color != null) {
                    if (COLOR_TO_LEGACY_FORMAT.containsKey(color)) {
                        out.append(COLOR_TO_LEGACY_FORMAT.get(color));
                    } else {
